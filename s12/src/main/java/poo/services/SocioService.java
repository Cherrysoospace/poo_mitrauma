...
public class SocioService implements Service<Socio> {
    ...
    private final JSONObject archivos;

    public SocioService() throws Exception {
        fileName = Utils.getConfig("archivos").getString("socios");
        archivos = Utils.getConfig("archivos");

        if (Utils.fileExists(archivos.getString("socios"))) {
            load();
        } else {
            list = new ArrayList<>();
        }
    ...
    public JSONObject get(String id) throws Exception {
       Socio c = getItem(id);
       if (c == null) {
        throw new NoSuchElementException(String.format("No se encontro un socio con ID %s", id));
       }
    ...
    @Override
    public List<Socio> load() throws Exception {
      list = new ArrayList<>();

      JSONArray jsonArr = new JSONArray(Utils.readText(archivos.getString("socios")));
      for (int i = 0; i < jsonArr.length(); i ++){
        JSONObject jsonObject = jsonArr.getJSONObject(i);
        list.add(new Socio(jsonObject));
      }
    ...
    public JSONObject remove(String id) throws Exception {
        ...
        Utils.writeJSON(list, archivos.getString("socios"));
    ...
    @Override
    public Socio dataToAddOk(String strJson) throws Exception {
    ...
    @Override
    public Socio getUpdated(JSONObject newData, Socio current) throws Exception {
